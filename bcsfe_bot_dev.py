...
async def start1(ctx):
	if ctx.message.channel.id == 1093028501990408255:
		...
		if msg.content == "1":
			try:
				...
				BCSFE_Python.helper.set_save_path("./bc_saves/{}/SD_{}".format(ctx.message.author.id, savetime))
...
async def mainmenu(ctx, save_stats):
		try:
